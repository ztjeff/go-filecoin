...
var log = logging.Logger("/fil/hello/1.0.0")
...
func init() {
 	encoding.RegisterIpldCborType(LoLoBigIntAtlasEntry)
	encoding.RegisterIpldCborType(LoLoHellolo{})
}

type LoLoBigInt struct {
 	*big.Int
}

var LoLoBigIntAtlasEntry = atlas.BuildEntry(LoLoBigInt{}).Transform().
	TransformMarshal(atlas.MakeMarshalTransformFunc(
		func(l LoLoBigInt) ([]byte, error) {
			return l.CborBytes(), nil
		})).
	TransformUnmarshal(atlas.MakeUnmarshalTransformFunc(
		func(x []byte) (LoLoBigInt, error) {
			return FromCborBytes(x)
		})).
	Complete()

func (bi *LoLoBigInt) CborBytes() []byte {
	if bi.Int == nil {
		return []byte{}
	}
...
func FromCborBytes(buf []byte) (LoLoBigInt, error) {
	if len(buf) == 0 {
		return LoLoBigInt{big.NewInt(0)}, nil
	}
	...
	case 1:
		negative = true
	default:
		return LoLoBigInt{}, fmt.Errorf("big int prefix should be either 0 or 1, got %d", buf[0])
	}
	...
	return LoLoBigInt{i}, nil
}



type LoLoHellolo struct {
 	HeaviestTipSet []cid.Cid
	HeaviestTipSetWeight LoLoBigInt
	GenesisHash cid.Cid
...
// var LoLoHelloloAtlasEntry = atlas.BuildEntry(LoLoHellolo{}).Transform().
// 	TransformMarshal(atlas.MakeMarshalTransformFunc(
// 		func(h LoLoHellolo) ([]byte, error) {
// 			enc := encoding.NewIpldCborEncoder()
// 			err := enc.EncodeArray(h.HeaviestTipSetCids)
// 			err = enc.EncodeStruct(h.HeaviestTipSetWeight)
// 			err = enc.EncodeStruct(h.GenesisHash)
// 			arrBs := enc.Bytes()
// 			// this is an array of length 3, prefix with 0x83
// 			arrBs = append([]byte{0x83}, arrBs...)
// 			return append(tsCidsBs, tsWeightBs...), nil
// 		})).
// 	TransformUnmarshal(atlas.MakeUnmarshalTransformFunc(
// 		func(x []byte) (LoLoHellolo, error) {
// 			return FromCborBytes(x)
// 		})).
// 	Complete()
...
//	return protocol.ID(fmt.Sprintf("/filecoin/hello/%s", networkName))
	return protocol.ID("/fil/hello/1.0.0")
}
...
func (h *HelloProtocolHandler) handleNewStream(s net.Stream) {
	fmt.Printf("handle new stream\n")
	defer func() {s.Close(); fmt.Printf("just closed stream from handler\n")}() // nolint: errcheck
//	p := s.Conn().RemotePeer()
	ctx := context.Background()
	_, err := h.receiveHello(ctx, s)
	if err != nil {
		fmt.Printf("bad receive hello %s\n", err)
		return 
	}
...
//	if err != nil {
//		fmt.Printf("erroring on the stream: %s\n", err)
//		return nil, err
//	}
	...
	// 	return nil, err
	// }
	// fmt.Printf("byte stream (n=%d): %x\n", n, buf[:n])
	// dec := encoding.NewIpldCborDecoder(buf[:n])
	// err = dec.DecodeValue(&hello)
	// if err != nil {
	// 	fmt.Printf("error decoding hello cbor format: %s\n", err)
	// } else {
	// 	fmt.Printf("hello obj: %v\n", hello)
	// }

	var hello LoLoHellolo
	if err := cbu.NewMsgReader(s).ReadMsg(&hello); err != nil {
		helloMsgErrCt.Inc(ctx, 1)
		fmt.Printf("error decoding with ipld cbor: %s\n", err)
		return nil, err
	}
	fmt.Printf("ipld cbor actually worked? %v\n", hello)

	return nil, fmt.Errorf("ahahahahaha")
}
...
func (hn *helloProtocolNotifiee) Connected(n net.Network, c net.Conn) {
	...
	// //	if err != nil {
	// //		fmt.Printf("received a bad hello message %s\n", err)
