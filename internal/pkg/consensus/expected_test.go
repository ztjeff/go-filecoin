...
func TestExpected_RunStateTransition_validateMining(t *testing.T) {
	...
	t.Run("passes the validateMining section when given valid mining blocks", func(t *testing.T) {
		...
		_, _, err = exp.RunStateTransition(ctx, tipSet, emptyBLSMessages, emptyMessages, emptyReceipts, []block.TipSet{pTipSet}, 0, blocks[0].StateRoot)
	...
	t.Run("returns nil + mining error when election proof validation fails", func(t *testing.T) {
		...
		_, _, err = exp.RunStateTransition(ctx, tipSet, emptyBLSMessages, emptyMessages, emptyReceipts, []block.TipSet{pTipSet}, 0, genesisBlock.StateRoot)
	...
	t.Run("fails when bls signature is not valid across bls messages", func(t *testing.T) {
		...
		_, _, err = exp.RunStateTransition(ctx, tipSet, blsMessages, emptyMessages, emptyReceipts, []block.TipSet{pTipSet}, 0, blocks[0].StateRoot)
		require.Error(t, err)
	...
	t.Run("fails when secp message has invalid signature", func(t *testing.T) {
		...
		_, _, err = exp.RunStateTransition(ctx, tipSet, emptyBLSMessages, secpMessages, emptyReceipts, []block.TipSet{pTipSet}, 0, blocks[0].StateRoot)
		require.Error(t, err)
