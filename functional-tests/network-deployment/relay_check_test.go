...
func TestRelayCheck(t *testing.T) {
	...
	testRelayCheck(ctx, t, env, f)
}

func testRelayCheck(ctx context.Context, t *testing.T, env environment.Environment, foo *Foo) {
	// TODO(tperson) Test only runs on devnets
	client, err := env.NewProcess(ctx, foo.PluginName, foo.PluginOptions, foo.FastOptions)
	...
	t.Run("Check for relay providers", func(t *testing.T) {
		...
		relayPeersAddrs := networkRelayPeers(foo.Network)
